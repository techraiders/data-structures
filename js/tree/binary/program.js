...
=> undefined
Invoke the callback for every node in depth-first post-order (visits the current node after its child node)
...
bsTree.checkIfFull()
=> true/false
A binary tree is full if every node has either zero or two children (no nodes have onew child)

bsTree.checkIfBalanced ()
=> true/false

*** Additional Exercide:
A binary search tree was created by iterating over an array and inserting each element into tree. Given a binary search tree with no duplicates, how many different arrays would result in the creation of this tree.
